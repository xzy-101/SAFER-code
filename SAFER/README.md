...
The core logic is located in the `SAFER/code/` directory, which includes `RM`, `sae`, and `data_processing` modules. The workflow is executed through three sequential stages:
1. **Stage 1 (Initial RM Training):** Run `SAFER/code/stage1.sh`
2. **Stage 2 (SAE Processing):** Run `SAFER/code/stage2.sh`
3. **Stage 3 (Poisoning & Retraining):** Run `SAFER/code/stage3.sh`

## Data and Model Preparation

### 1. Download Dataset
Please download the essential dataset for this project from the link below and place it in the `SAFER/Dataset` directory. Ensure that the directory contains two subfolders: `RM` and `SAE`.
...
### 2. Download Model
Download the base model and place it in the `SAFER/model` directory. Ensure the folder name is `Llama-3.2-1B-Instruct`.
