...
def create_dataloader(
    ...
    dataset = SafetyDataset(folder_path, tokenizer, max_length, keyword)
...
def get_outputs(
    ...
    return input_ids, attention_mask, outputs, selected_hidden_states, punctuation_indices, k
    ...
    def run(self):
        ...
        for epoch in range(self.cfg.num_epochs):
            ...
            for batch_idx, batch in enumerate(self.dataloader):
                _, _, _, hidden_states, _, _= get_outputs(self.cfg, batch, self.language_model, self.device)
        ...
        if self.cfg.use_wandb:
            wandb.finish()
        unit_norm_decoder(self.model)
        torch.save(self.model.state_dict(), f'code/sae/SAE_models/{self.title}.pt')
        return curr_loss
...
class Evaluater:
    ...
    def run(self):
        ...
        print(len(self.dataloader))
        for batch_idx, batch in enumerate(self.dataloader):
            input_ids, attention_mask, outputs, hidden_states, _, _ = get_outputs(self.cfg, batch, self.language_model, self.device)
            x, _, _ = pre_process(hidden_states)
            _, x_hat = self.model(x)
...
class Applier_ChosenActivation:
    ...
    def get_context(
        ...
        with open("code/sae/src/chosen.txt", "w") as f:
...
class Applier_RejectedActivation:
    ...
    def get_context(
        ...
        with open("code/sae/src/rejected.txt", "w") as f:
            for idx, value in enumerate(activation_sums.tolist()):
                f.write(f"{idx}\t{value}\n")
...
class Applier_Context:
    ...
    def get_context(
        ...
        if output_path is None:
            output_path = f'code/sae/context/context.json'
        ...
        self.tokenizer, self.language_model = get_language_model(self.cfg.model_path, self.device)
        dataloader = create_dataloader(self.cfg.data_path, self.tokenizer, self.cfg.batch_size, self.cfg.max_length)
        file_path = "code/sae/src/top_latents.txt"
        ...
        for batch_idx, batch in enumerate(dataloader):
            input_ids, _, _, hidden_states, punctuation_indices, _ = get_outputs(self.cfg, batch, self.language_model, self.device)
            x, _, _ = pre_process(hidden_states)
            latents, _ = self.model(x)
...
class Applier_DataScore:
    ...
    def get_context(
        ...
        safelatent_dict = {}
        unsafelatent_dict = {}
        prefix = "code/sae/src/top_latents"
        for limit in limits:
                    ...
                    for limit in limits:
                        ...
                        if latent_dim in safelatent:
                            if y == 1:
                                ...
                                sr_dict[limit][idx] += activation_value
        for limit in limits:
            file_path = "code/sae/src/SafetyScore.txt"
            print(file_path)
...
class Interpreter:
    ...
    def run(
        ...
        if output_path is None:
            output_path = f'code/sae/interpret/interpret.json'
...
class SAE_pipeline:
    def __init__(self, cfg):
        self.cfg = cfg
        self.cfg.SAE_path = f'code/sae/SAE_models/stage2.pt'
        self.result_dict = {}
